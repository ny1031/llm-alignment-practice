...
MATH_DATASET="${DATA_DIR:-/path/to/datasets}/RLVR-MATH"
GSM_DATASET="${DATA_DIR:-/path/to/datasets}/RLVR-GSM"
...
LOSS_TYPE="grpo"
TEMPERATURE=0.8
MATH_EVAL_RATIO=0.05
EVAL_STEPS=100

# Load API keys from .env
source "$WORKDIR/.env"
export WANDB_API_KEY
export WANDB_PROJECT="lmalign-2026-rlvr-grpo-math"

DATASET_NAME_SHORT="$(basename ${MATH_DATASET})+$(basename ${GSM_DATASET})"
...
accelerate launch --num_processes=7 --gpu_ids="0,1,2,3,4,5,6" grpo_math.py \
    --model_name $MODEL_NAME \
    --math_dataset $MATH_DATASET \
    --gsm_dataset $GSM_DATASET \
    ...
    --use_vllm \
    --vllm_mode "server" \
    --math_eval_ratio $MATH_EVAL_RATIO \
